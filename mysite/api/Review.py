...
async def create_review(user: ReviewInputSchema, db: Session = Depends(get_db)):
...
async  def detail_category(review_id: int, db: Session = Depends(get_db)):
...
async def update_review(review_id: int,user: ReviewInputSchema,
                          db: Session = Depends(get_db)):
    review_db = db.query(Review).filter(Review.id==review_id).first()
    if not review_db:
        raise HTTPException(detail='Мындай маалымат жок', status_code=400)

    for review_key, review_value in user.dict().items():
...
async def delete_review(review_id, db: Session = Depends(get_db)):
    review_db = db.query(Review).filter(Review.id==review_id).first()
    if not review_db:
        raise HTTPException(detail='Мындай маалымат жок', status_code=400)
