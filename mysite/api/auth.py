...
async def register(user: UserProfileInputSchema, db: Session = Depends(get_db)):
    ...
    if user_db:
        raise HTTPException(detail='мындай юзер бар башка жазыныз', status_code=400)
    elif email_db:
        raise HTTPException(detail='мындай почта бар башка жазыныз', status_code=400)
    ...
    return {'massage': 'Ийгиликтуу аяктады'}
...
async def login(user:UserLoginSchema, db:Session = Depends(get_db)):
    ...
    token_db = RefreshToken(profile_id=user_db.id, token=refresh_token)
    db.add(token_db)
    db.commit()
...
async def logout(refresh_token:str,db: Session = Depends(get_db)):
    ...
    if not stored_token:
        raise  HTTPException(status_code=401, detail='маалымат туура эмес туура жаз')
